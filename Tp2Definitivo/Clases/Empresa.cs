...
namespace Tp2Definitivo
{
    ...
    public class Empresa : IExportable
    {
        ...
        public bool HacerReserva(int dni, int nro, DateTime ingreso, DateTime egreso)
        {
            ...
            Reserva nueva = new Reserva(cliente, propiedad, DateTime.Now, ingreso, egreso, cantidadReservas++);
            if (nueva != null)
            {
        ...
        public void RetirarReserva(Reserva reserva)
        {
            try
            {
                ticketsEmitidos.Add(reserva);
                EliminarReserva(reserva);
                Propiedad propiedad = BuscarPropiedad(reserva.NroPropiedad);
                propiedad.EliminarReserva(reserva.Nro);
            }
            catch (Exception ex)
            {
